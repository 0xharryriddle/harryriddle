import type { TutorialMetadata, TutorialDifficulty } from "@/app/community/utils";
...
function StatusIndicator({ status }: { status: string }) {
  ...
  return (
    <span className="flex items-center gap-1.5 text-xs text-[var(--text-muted)]">
      <span
        className={`w-1.5 h-1.5 rounded-full ${colors[status] || "bg-neutral-400"}`}
      />
      {status}
    </span>
  );
